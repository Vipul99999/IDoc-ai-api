...
export async function POST(request: Request) {
  ...
  const quota = await checkQuota(principal, "ocr_page", document.pageCount);
  if (!quota.ok) return v1Error(request, principal.tenantId, 429, "quota_exceeded", "Monthly OCR quota exceeded.", quota);
  ...
  const queueJob = await enqueueJob({
    ...
    status: "queued",
    progress: 5,
    message: "Queued for OCR text and coordinate extraction.",
    attempts: 0,
    maxAttempts: 3,
    payload: {},
    createdAt: now,
